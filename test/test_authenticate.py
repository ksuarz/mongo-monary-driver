...
def setup():
    global db
    assert '--auth' in pymongo.MongoClient().admin.command('getCmdLineOpts')['argv'], \
           'The mongo server (mongod) needs to be running with authentication (--auth)'
...
def test_with_authenticate():
    ...
    [ col ] = monary_connection.query("monary_auth_test", "junk", {}, ["route"], ["int32"])
    print col
    assert col[0] == 66, "test value could not be retrieved"

def test_with_authenticate_from_uri():
    monary_connection = monary.Monary("mongodb://monary_test_user:monary_test_pass@"
